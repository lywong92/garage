...
class GaussianMLPModule(nn.Module):
    ...
    Args:
        ...
        init_std (float): Initial value for std (normal value).
        ...
        min_std (float): If not None, the std is at least the value of min_std,
            to avoid numerical issues (normal value).
        max_std (float): If not None, the std is at most the value of max_std,
            to avoid numerical issues (normal value).
        std_hidden_nonlinearity: Nonlinearity for each hidden layer in
            the std network.
        std_output_nonlinearity (callable): Activation function for output
            dense layer in the std network. It should return a torch.Tensor. Set
    ...
    def __init__(self,
        ...
        self._init_std_param = torch.tensor(init_std, dtype=torch.float, requires_grad=self._learn_std)

        self._min_std_param, self._max_std_param = None, None
        if min_std:
            min_std = torch.tensor(min_std, dtype=torch.float)
        if max_std:
            max_std = torch.tensor(max_std, dtype=torch.float)
        ...
        if self._std_parameterization == 'exp':
            ...
            self._init_std_param = self._init_std_param.exp().add(-1).log()
    ...
    def forward(self, inputs):
        if self._std_share_network:
            ...
            log_std_uncentered = output[..., self._action_dim:]
        else:
            mean = self.mean_module(inputs)

            if self._adaptive_std:
                log_std_uncentered = self.adaptive_std_module(inputs)
            else:
                broadcast_shape = list(inputs.shape[:-1]) + [self._action_dim]
                x = torch.zeros(*broadcast_shape)
                log_std_uncentered = x + self._init_std_param

        if self._min_std_param or self._max_std_param:
            log_std_uncentered = log_std_uncentered.clamp(
                min=to_scalar_if_not_none(self._min_std_param),
                max=to_scalar_if_not_none(self._max_std_param))

        if self._std_parameterization == 'exp':
            log_std = log_std_uncentered
        else:
            log_std = log_std_uncentered.exp().exp().add(1.).log().log()

        rnd = torch.rand(list(mean.shape)[1:])
        action = rnd * log_std.exp() + mean

        cov = (log_std.exp() ** 2).diag_embed()
        dist = MultivariateNormal(mean, cov)

        return action, mean, log_std, log_std_uncentered, dist


def to_scalar_if_not_none(tensor):
